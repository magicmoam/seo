...
export function render(r) {
  ...
  if (r.phases || r.website_analysis || r.keyword_research || r.executive_summary) {
    if (r.executive_summary) {
      h += `<div class="panel"><div class="panel-label">Executive Summary</div><p style="font-size:14px;font-weight:300;line-height:1.7;color:var(--c-text-secondary)">${esc(r.executive_summary)}</p></div>`;
    }
  ...
  h += `<div class="panel"><div class="panel-label">Summary</div><p style="font-size:14px;font-weight:300;line-height:1.7;color:var(--c-text-secondary)">${esc(r.summary || r.executive_summary || '')}</p></div>`;
  return h;
}
